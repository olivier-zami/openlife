...
#include "src/server/feature/genetics.h"
#include "src/server/feature/leader.h"
#include "src/server/feature/peaceTreaty.h"
...
extern volatile char quit;
extern float targetHeat;
extern double secondsPerYear;
extern int minPickupBabyAge;
extern int babyAge;
extern int defaultActionAge;// age when bare-hand actions become available to a baby (opening doors, etc.)
extern double startWalkingAge;// can't walk for first 12 seconds
extern double forceDeathAge;
extern double minSayGapInSeconds;
extern double emoteWindowSeconds;// for emote throttling
extern int maxEmotesInWindow;
extern double emoteCooldownSeconds;
extern int maxLineageTracked;// each generation is at minimum 14 minutes apart // so 1024 generations is approximately 10 days
...
extern double apocalypseStartTime;
extern char apocalypseStarted;
extern char postApocalypseStarted;
extern double remoteApocalypseCheckInterval;
extern double lastRemoteApocalypseCheckTime;
...
extern int longestShutdownLine;

extern double lastPastPlayerFlushTime;
extern double deadlyMapSpotTimeoutSec;
extern double minFlightSpeed; // min speed for takeoff
extern double playerCrossingCheckStepTime;// how often do we check what a player is standing on top of for attack effects?
extern double periodicStepTime;// for steps in main loop that shouldn't happen every loop// (loop runs faster or slower depending on how many messages are incoming)
extern double lastPeriodicStepTime;
extern int numPlayersRecomputeHeatPerStep;
extern int lastPlayerIndexHeatRecomputed;
extern double lastHeatUpdateTime;// recompute heat for fixed number of players per timestep
extern double heatUpdateTimeStep;
extern double heatUpdateSeconds;// how often the player's personal heat advances toward their environmental// heat value
extern float rAir;// air itself offers some insulation// a vacuum panel has R-value that is 25x greater than air
extern char eveWindowOver;

extern SimpleVector<float> recentScoresForPickingEve;
extern SimpleVector<char *>recentScoreEmailsForPickingEve;
...
extern JenkinsRandomSource curseSource;


extern double minFoodDecrementSeconds;
extern double maxFoodDecrementSeconds;
extern double foodScaleFactor;
extern double foodScaleFactorFloor;
extern double foodScaleFactorHalfLife;
extern double foodScaleFactorGamma;
extern double newPlayerFoodDecrementSecondsBonus;
extern int newPlayerFoodEatingBonus;
extern double newPlayerFoodBonusHalfLifeSeconds;// first 10 hours of living
extern double indoorFoodDecrementSecondsBonus;
extern int babyBirthFoodDecrement;
extern int babyFeedingLevel;
extern int nurseCost;// fixed cost to pick up baby// this still encourages baby-parent// communication so as not// to get the most mileage out of// food
extern int eatBonus;// bonus applied to all foods// makes whole server a bit easier (or harder, if negative)
extern double eatBonusFloor;
extern double eatBonusHalfLife;
extern double eatCostMax;
extern double eatCostGrowthRate;
extern int canYumChainBreak;// -1 for no cap
extern int yumBonusCap;
extern double minAgeForCravings;
extern double posseSizeSpeedMultipliers[4];
extern double killerVulnerableSeconds;
...
extern int versionNumber;// larger of dataVersionNumber.txt or serverCodeVersionNumber.txt
extern double childSameRaceLikelihood;
extern int familySpan;
extern SimpleVector<char*> nameGivingPhrases;// phrases that trigger baby and family naming
extern SimpleVector<char*> familyNameGivingPhrases;
extern SimpleVector<char*> eveNameGivingPhrases;
...
extern SimpleVector<char*> familyGivingPhrases;
extern SimpleVector<char*> offspringGivingPhrases;
extern SimpleVector<char*> posseJoiningPhrases;
extern SimpleVector<char*> youFollowPhrases;
extern SimpleVector<char*> namedFollowPhrases;
extern SimpleVector<char*> youExilePhrases;
extern SimpleVector<char*> namedExilePhrases;
extern SimpleVector<char*> youRedeemPhrases;
extern SimpleVector<char*> namedRedeemPhrases;
extern SimpleVector<char*> youKillPhrases;
extern SimpleVector<char*> namedKillPhrases;
extern SimpleVector<char*> namedAfterKillPhrases;
extern SimpleVector<WarState> warStates;
extern char *orderPhrase;
extern char *eveName;
extern char allowedSayCharMap[256];// maps extended ascii codes to true/false for characters allowed in SAY// messages
extern const char *allowedSayChars;
extern int killEmotionIndex;
extern int victimEmotionIndex;
extern int victimTerrifiedEmotionIndex;
extern int starvingEmotionIndex;
extern int satisfiedEmotionIndex;
extern double lastBabyPassedThresholdTime;
extern int recentScoreWindowForPickingEve;
...
extern double minGlobalMessageSpacingSeconds;// messages with no follow-up hang out on client for 10 seconds// 7 seconds should be long enough to read if there's a follow-up waiting
extern SimpleVector<DeadlyMapSpot> deadlyMapSpots;
extern double diagLength;// diags are square root of 2 in length
extern SimpleVector<char *> curseWords;
extern char *curseSecret;
extern SimpleVector<char *> familyNamesAfterEveWindow;
extern SimpleVector<int> familyLineageEveIDsAfterEveWindow;
extern SimpleVector<int> familyCountsAfterEveWindow;
extern int nextBabyFamilyIndex;
extern FILE *postWindowFamilyLogFile;
extern double shortLifeAge;// keep track of players whose last life was short
extern SimpleVector<char*> shortLifeEmails;
extern double minPosseFraction;
extern int minPosseCap;
extern double possePopulationRadius;
extern int cursesUseSenderEmail;
extern int useCurseWords;
extern int pathDeltaMax;
extern double baseWalkSpeed;// with 128-wide tiles, character moves at 480 pixels per second// at 60 fps, this is 8 pixels per frame// important that it's a whole number of pixels for smooth camera movement
extern int chunkDimensionX;
extern int chunkDimensionY;
extern int maxSpeechRadius;
extern SocketPoll sockPoll;
extern int orderDistance;
extern double killDelayTime;
extern double posseDelayReductionFactor ;
extern int maxPlacementX;// for placement of tutorials out of the way
extern int tutorialOffsetX;// tutorial is alwasy placed 400,000 to East of furthest birth/Eve// location
extern int tutorialCount;// each subsequent tutorial gets put in a diferent place
...
extern char nextLogInTwin;
extern int firstTwinID;
		...
		char* anyBiomesInDB)
{
	...
	this->shutdownMode = serverSettings.global.shutdownMode;
	this->forceShutdownMode = serverSettings.global.forceShutdownMode;
	this->someClientMessageReceived = serverSettings.global.someClientMessageReceived;
	this->nextOrderNumber = 1;
	this->lastPastPlayerFlushTime = 0;
	this->deadlyMapSpotTimeoutSec = 10;
	this->minFlightSpeed = 15;
	this->playerCrossingCheckStepTime = 0.25;
	this->periodicStepTime = 0.25;
	this->lastPeriodicStepTime = 0;
	this->numPlayersRecomputeHeatPerStep = 8;
	this->lastPlayerIndexHeatRecomputed = -1;
	this->lastHeatUpdateTime = 0;
	this->heatUpdateTimeStep = 0.1;
	this->heatUpdateSeconds = 2;
	this->rAir = 0.04;
...
void openLife::Server::start()
{
	...
	while( !quit )
	{
		...
		if( curStepTime - this->lastPastPlayerFlushTime > 3600 )
		{
			...
			this->lastPastPlayerFlushTime = curStepTime;
		}

//!
		char periodicStepThisStep = false;
		if( curStepTime - lastPeriodicStepTime > this->periodicStepTime )
		{
		...
		if( periodicStepThisStep )
		{
			...
			*(this->forceShutdownMode) = SettingsManager::getIntSetting( "forceShutdownMode", 0 );
			...
			if(!openLife::system::isFileWritable()) {
				...
				*(this->shutdownMode) = 1;
				*(this->forceShutdownMode)  = 1;
			}
		}

//!check shutdown + force shutdown?
		if( *(this->forceShutdownMode)  )
		{
			*(this->shutdownMode) = 1;
...
//!
		...
		// frequency
		double pollTimeout = this->periodicStepTime;
		...
		*(this->someClientMessageReceived) = false;
		...
		for( int i=0; i<numLive; i++ ) {
			...
			if( curCrossTime < nextPlayer->playerCrossingCheckTime +
							   this->playerCrossingCheckStepTime ) {
...
/**********************************************************************************************************************/
		...
		if( currentTimeHeat - this->lastHeatUpdateTime >= heatUpdateTimeStep ) {
			// a heat step has passed


			// recompute heat map here for next players in line
			int r = 0;
			for( r=this->lastPlayerIndexHeatRecomputed+1;
				 r < this->lastPlayerIndexHeatRecomputed + 1 +
					 this->numPlayersRecomputeHeatPerStep
				 &&
				 r < players.size(); r++ ) {
			...
			this->lastPlayerIndexHeatRecomputed = r - 1;
			...
			this->lastHeatUpdateTime = currentTimeHeat;
		}
		...
		for( int i=0; i< players.size(); i++ ) {
			...
			if( nextPlayer->error ||
				currentTime - nextPlayer->lastHeatUpdate < this->heatUpdateSeconds ) {
				continue;
			}
...
void updateYum( LiveObject *inPlayer, int inFoodEatenID,
					   char inFedSelf )
{
	char wasYummy = true;
 ...
 * @param inPartial
 * @return
 * @note inDelete true to send X X for position
 ...
 * @param outHitIndex
 * @return
 * @note if inTargetID set, we only detect whether inTargetID is close enough to be hit
 * otherwise, we find the lowest-id player that is hit and return that
 */
LiveObject *getHitPlayer( int inX, int inY,
								 int inTargetID,
								 char inCountMidPath,
								 int inMaxAge,
								 int inMinAge,
 ...
 * @param inRB
 * @return
 * @note blend R-values multiplicatively, for layers
 * 1 - R( A + B ) = (1 - R(A)) * (1 - R(B))
 * or
 ...
 * @param inSwap
 * @return
 * @note swap indicates that we want to put the held item at the bottom
 * of the container and take the top one
 * returns true if added
 */
char addHeldToContainer( LiveObject *inPlayer,
								int inTargetID,
 ...
 * @return
 * @note outCouldHaveGoneIn, if non-NULL, is set to TRUE if clothing
 * could potentialy contain what we're holding (even if clothing too full to contain it)
 */
 ...
 * @param inContY
 * @return
 * @note find index of spot on container held item can swap with, or -1 if none found
 */
 ...
 * @param inContainedID
 * @return
 * @note checks for granular +cont containment limitations
...
void pickupToHold( LiveObject *inPlayer, int inX, int inY,
						  int inTargetID )
{
	inPlayer->holdingEtaDecay =
			getEtaDecay( inX, inY );
 ...
 * @param inI
 * @return
 * @note returns true if it worked
 */
char removeFromClothingContainerToHold( LiveObject *inPlayer,
											   int inC,
											   int inI = -1 ) {
...
TransRecord *getBareHandClothingTrans( LiveObject *nextPlayer,
											  ObjectRecord **clothingSlot )
{
 ...
 * @param inPlayer
 * @param inEatenID
 * @note inEatenID = 0 for nursing
 */
void checkForFoodEatingEmot( LiveObject *inPlayer,
									int inEatenID ) {
...
void sendHomelandMessage( LiveObject *nextPlayer,
								 int homeLineageEveID,
								 GridPos homeCenter )
{
 ...
 * @param inHoldingID
 * @return
 * @note access blocked b/c of access direction or ownership?
 */
char isAccessBlocked( LiveObject *inPlayer,
 ...
 */
LiveObject *getPlayerByName( char *inName,
...
void findExpertForPlayer( LiveObject *inPlayer,
 ...
 * @param outLength
 * @return
 * @note if inAll, generates info for all players, and doesn't touch followingUpdate flags returns NULL if no following message
 */
...
void tryToStartKill( LiveObject *nextPlayer, int inTargetID,
							SimpleVector<int> *playerIndicesToSendUpdatesAbout,
							char inInfiniteRange )
{
	if( inTargetID > 0 &&
		nextPlayer->holdingID > 0 ) {
 ...
 * @param inY
 * @return
 * @note check if target has a 1-second decay specified if so, make it happen NOW and set in map return new target id
 */
...
void setHeldGraveOrigin( LiveObject *inPlayer, int inX, int inY,
								int inNewTarget )
{
 ...
 * @return
 * @note result NOT destroyed by caller can be NULL if not found
 */
...
/**
 *
 * @param inTargetPlayer
 * @param playerIndicesToSendUpdatesAbout
 * @note does not force-drop of player holding wound, sickness, etc.
 */
void tryToForceDropHeld(
...
void replaceOrder( LiveObject *nextPlayer, char *inFormattedOrder,
 ...
 * @param listenerObj
 * @return
 * @note speaker can be NULL sometimes
 */
char *translatePhraseFromSpeaker( char *inPhrase,
										 LiveObject *speakerObj,
...
void sendLearnedToolMessage( LiveObject *inPlayer,
...
// result NOT destroyed by caller
					const char *getCurseWord( char *inSenderEmail,
...
GridPos computePartialMoveSpot( LiveObject *inPlayer, int inOverrideC)
{
...
void setDeathReason( LiveObject *inPlayer, const char *inTag,
int inOptionalID )
{
...
void setPlayerDisconnected( LiveObject *inPlayer,
const char *inReason )
{
/*
...
void forcePlayerToRead( LiveObject *inPlayer,
int inObjectID )
{
...
void sendToolExpertMessage( LiveObject *inPlayer,
...
void holdingSomethingNew( LiveObject *inPlayer,
int inOldHoldingID)
{
if( inPlayer->holdingID > 0 ) {
...
int isGraveSwapDest( int inTargetX, int inTargetY,
...
int isPlayerCountable( LiveObject *p, int inLineageEveID = -1,
int inRace = -1 )
{
if( p->error ) {
return false;
...
// ONLY in that family
int countFertileMothers( int inLineageEveID = -1, int inRace = -1 )
{
...
// ONLY in that family
int countGirls( int inLineageEveID = -1, int inRace = -1 )
{
...
int countNonHelpless( GridPos inPos, double inRadius,
double inMinAge = 0 )
{
int c = 0;
...
int countAllies( LiveObject *inPlayer,
...
int countEnemies( LiveObject *inPlayer,
GridPos inPos, double inRadius )
{
int count = 0;
...
char isMapSpotBlockingForPlayer( LiveObject *inPlayer,
int inX, int inY )
{
...
char directLineBlocked( LiveObject *inShooter,
...
char isWildcardGivingSay( char *inSaidString,
...
int countPosseSize( LiveObject *inTarget,
int *outMinPosseSizeForKill,
char *outFullForceSoloPosse)
{
	int p = 0;
...
void updatePosseSize( LiveObject *inTarget,
...
void interruptAnyKillEmots( int inPlayerID,
...
void setPerpetratorHoldingAfterKill( LiveObject *nextPlayer,
TransRecord *woundHit,
TransRecord *rHit,
TransRecord *r )
{
...
void replaceNameInSaidPhrase( char *inSaidName, char **inSaidPhrase, LiveObject *inNamedPerson, char inForceBoth )
{
	char *trueName;
